...
function generateRandomEventTime(){
    const times = ['1pm', '2pm', '4pm', '6:30pm', '7:45pm', '10:15pm', '11am', '10:45am'];
    ...
    describe('/api/events GET endpoint',function(){
        it('should return all events for logged in user', function(){
            ...
            .then(_res => {
                ...
                return Events.find({user: username})
                .count();
            }).then(count => {
    ...
    describe('PUT request', function(){
        ...
        it('should correctly update fields on PUT request', function(){
            ...
            .then(event => {
                ...
                .then(event => {
                    expect(event.contact.firstName).to.equal(updatedEvent.contact.firstName);
                    expect(event.contact.lastName).to.equal(updatedEvent.contact.lastName);
                    expect(event.time).to.equal(updatedEvent.time);
                    expect(event.order.rentalPrice).to.equal(updatedEvent.order.rentalPrice);
                })
            })
        })
    });
